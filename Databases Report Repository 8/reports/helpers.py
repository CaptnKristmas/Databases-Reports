...
def run_sql_and_return_df(cnx, sql, show_size=True):
    ...
    try:
        ...
        if isinstance(orig, pymysql.err.OperationalError) and orig.args[0] == 1044:
            errors.append(
                f"<hr/><b>Access denied while connecting</b><br/>"
                f"{orig.args[1]}<br/>"
                "→ Check your .env credentials (username/password) and verify you have privileges "
                "for the specified database."
            )
        else:
            errors.append(f"<hr/><b>OperationalError connecting</b><br/>{str(e)}<br/>")
...
def execute_ddl(cnx, ddl_commands):
    ...
    try:
        # ---- OUTER connection block ----
        with cnx.connect() as connection:
            for statement in ddl_statements:
                try:
                    result = connection.execute(text(statement))
                    result_info = (
                        result.rowcount if result.rowcount != -1
                        else "No rows affected"
                    )
                    messages.append(
                        f"Executed statement: {statement}<br/>Result: {result_info}<br/>"
                    )

                # ---- INNER per-statement OperationalError ----
                except OperationalError as e:
                    orig = getattr(e, "orig", None)
                    if isinstance(orig, pymysql.err.OperationalError) and orig.args[0] == 1044:
                        errors.append(
                            f"<hr/><b>Access denied</b><br/>"
                            f"User does not have permission to the target database.<br/>"
                            f"Error: {orig.args[1]}<br/>"
                            "→ Check your .env credentials and ensure the database name matches your user."
                        )
                    else:
                        errors.append(
                            f"<hr/>OperationalError executing: <b>{statement}</b><br/>"
                            f"Error: {str(e)}<br/>"
                        )

                except Exception as e:
                    errors.append(
                        f"<hr/>Error executing statement: <b>{statement}</b><br/>Error: {str(e)}<br/>"
                    )

    # ---- OUTER OperationalError (connect-time) ----
    except OperationalError as e:
        orig = getattr(e, "orig", None)
        if isinstance(orig, pymysql.err.OperationalError) and orig.args[0] == 1044:
            errors.append(
    ...
    # ---- Any other unexpected outer exception ----
    except Exception as e:
        errors.append(f"<hr/><b>Error connecting</b><br/>Error: {str(e)}<br/>")
        if "1044" in str(e):
            errors.append("CHECK YOUR CREDENTIALS in your .env file!")
...
def execute_ddl_from_file(filename, cnx):
    """
    Loads DDL statements from a file and executes them on the given SQLAlchemy connection
    using execute_ddl(). Handles missing or unreadable files gracefully.

    Returns:
        None if execution succeeded without errors,
        or a DataFrame (as HTML) if errors occurred.
    """
    ...
    if not os.access(filename, os.R_OK):
        error_message = f"File is not readable: {filename}"
        df = pd.DataFrame({
    ...
    # --- Delegate to execute_ddl ---
    try:
        return execute_ddl(cnx, ddl_commands)
    except Exception as e:
        error_message = f"Error executing DDL from file '{filename}': {str(e)}"
        df = pd.DataFrame({
